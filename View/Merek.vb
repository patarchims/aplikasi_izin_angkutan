...
Public Class Merek
    ...
    Sub TampilGrid()
        ...
        Dim Adapter As New MySqlDataAdapter(" Select * FROM merek  where " & _
        "nm_merek like '" & Trim(txtcari.Text) & "%' order by kd_merek", conn)
        Adapter.Fill(table)
        GrdView.DataSource = table
        GrdView.Refresh()
        tutupkoneksi()
        Me.GrdView.Columns(0).HeaderText = "Kode Merek"
        Me.GrdView.Columns(1).HeaderText = "Nama Merek"
    End Sub
    Sub AturGrid()
    ...
    Sub KodeAuto()
        ...
        sqlcommand = New MySqlCommand("select * from merek order by kd_merek desc", conn)
        Dr = sqlcommand.ExecuteReader
        If Dr.Read Then
            strSementara = Mid(Dr.Item("kd_merek"), 4, 5)
            strIsi = Val(strSementara) + 1
            txtkode.Text = "MRK" + Mid("00000", 1, 5 - strIsi.Length) & strIsi
        Else
            txtkode.Text = "MRK00001"
        End If
        txtnama.Focus()
    End Sub
    ...
    Private Sub BtnSimpan_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSimpan.Click
        ...
        Else
            tutupkoneksi()
            cekkoneksi()
            sql = "SELECT * FROM merek  WHERE kd_merek='" & Trim(txtkode.Text) & "'"
            sqlcommand = New MySql.Data.MySqlClient.MySqlCommand(sql, conn)
            Dr.Close()
            Dr = sqlcommand.ExecuteReader()
            Dr.Read()
            If Not Dr.HasRows Then
                sql = "INSERT INTO merek values ('" & txtkode.Text & "','" & _
                     txtnama.Text & "')"
            Else
                sql = "UPDATE merek set nm_merek='" & txtnama.Text & "' WHERE kd_merek='" & Trim(txtkode.Text) & "'"
            End If
            sqlcommand = New MySql.Data.MySqlClient.MySqlCommand(sql, conn)
            tutupkoneksi()
            cekkoneksi()
    ...
    Private Sub BtnHapus_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnHapus.Click
        ...
        tutupkoneksi()
        cekkoneksi()
        sql = "SELECT * FROM merek where kd_merek='" & txtkode.Text & "'"
        sqlcommand = New MySql.Data.MySqlClient.MySqlCommand(sql, conn)
        Dr.Close()
        Dr = sqlcommand.ExecuteReader
        Dr.Read()
        ...
        If Message = Windows.Forms.DialogResult.Yes Then
            tutupkoneksi()
            sql = "Delete From merek where kd_merek ='" & txtkode.Text & "'"
            cekkoneksi()
            sqlcommand = New MySql.Data.MySqlClient.MySqlCommand(sql, conn)
            sqlcommand.ExecuteNonQuery()
            sqlcommand = New MySql.Data.MySqlClient.MySqlCommand(sql, conn)
